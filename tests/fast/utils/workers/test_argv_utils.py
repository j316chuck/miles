...
def _render(args_obj: _DemoArgs) -> list[str]:
    return render_cli_argv(
        dataclass_to_values(args_obj), wanted_obj=args_obj, make_parser=_make_parser, from_parsed=_from_parsed
    )
...
class TestRenderCliArgvBaselineFields:
    def test_a_baseline_field_is_emitted_exactly_once(self):
        """It seeds the defaults probe and stays in the final argv, so it must not be rendered twice."""
        args_obj = _RequiredDemoArgs(model="m", count=3)
        argv = _render_required(args_obj, baseline_fields=("model",))
        assert argv.count("--model") == 1
        assert _from_parsed_required(_make_required_parser().parse_args(argv)) == args_obj

    def test_a_baseline_field_is_emitted_even_at_its_own_default(self):
        """The parser rejects the defaults probe without it, default value or not."""
        assert _render_required(_RequiredDemoArgs(model="m"), baseline_fields=("model",)) == ["--model", "m"]

    def test_a_parser_with_required_flags_needs_baseline_fields(self):
        """Without them the defaults probe hits the missing required flag."""
        with pytest.raises(SystemExit):
            _render_required(_RequiredDemoArgs(model="m"))
