"""Trainer-side verbs for the tinker-compatible backend.
...
def cleanup_adapters(args, model, optimizer, adapters) -> int:
    ...
    if is_first_replica_megatron_main_rank():
        for adapter in adapters:
            ray.get(get_tinker_controller().free_slot.remote(adapter.name))
    return len(adapters)
...
def commit_weight_push(version_update_names: list, is_main_rank: bool) -> None:
    ...
    if version_update_names and is_main_rank:
        ray.get(get_tinker_controller().record_weight_update.remote(version_update_names))
