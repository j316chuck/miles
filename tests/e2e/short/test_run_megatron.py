...
def _prepare(dump_dir: Path, config: _ModeConfig) -> Path:
    ...
    _backend().exec_command_cpu("mkdir -p /root/models")
    _backend().exec_command_cpu(f"hf download {HF_REPO} --local-dir /root/models/{MODEL_NAME}")
    U.convert_checkpoint(
        model_name=MODEL_NAME,
        megatron_model_type=MODEL_TYPE,
        num_gpus_per_node=min(NUM_GPUS, NUM_LAYERS),
    )
    _backend().exec_command_cpu(f"rm -rf {dump_dir}")
