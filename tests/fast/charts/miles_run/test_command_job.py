...
ENABLE_COMMAND_JOB = (
    ...
    "--set",
    "command_job.objectName=myrun-miles-run-convert",
    "--set-json",
    'command_job.command=["bash","-c","convert"]',
)
    ...
    def test_lets_the_caller_choose_the_deadline_and_the_lifetime(self):
        ...
        job = single_object_of_kind(
            render_run(
                *ENABLE_COMMAND_JOB,
                "--set",
                "command_job.activeDeadlineSeconds=60",
                "--set",
                "command_job.ttlSecondsAfterFinished=0",
            ),
            "Job",
        )
