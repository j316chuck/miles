...
from miles.utils.external_utils.command_utils.helm_backend.launcher.observability import (
    cluster_info,
    log_follower,
    pod_facts,
)
...
class TestBackgroundLogOrdering:
    def test_the_watcher_runs_while_the_logs_stream(self, monkeypatch, tmp_path):
        ...
        pods = [pod_facts.PodStatus(name="p", phase="Running", containers=(_container(),))]
...
def _container() -> pod_facts.ContainerStatus:
    return pod_facts.ContainerStatus(name="orchestrator", container_id="docker://a", running=True)
