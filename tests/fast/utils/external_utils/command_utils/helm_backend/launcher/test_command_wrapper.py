...
class TestLogCommands:
    def test_follows_a_workload_by_container_name(self):
        """A pod may gain a sidecar, and kubectl then refuses to guess which container to read."""
        command = Kubectl.follow_workload_logs_command(
            namespace="rl", workload="r-miles-run-orchestrator", container="orchestrator"
        )

        assert command[-2:] == ["-c", "orchestrator"]
        assert "statefulset/r-miles-run-orchestrator" in command

    def test_follows_every_pod_a_selector_matches_rather_than_one_workload(self):
        """A worker that crashed is the reason the run stopped, and its log is in another workload entirely."""
        command = Kubectl.follow_selected_logs_command(namespace="rl", selector="a=b")

        assert command[command.index("--selector") + 1] == "a=b"
...
def _kubectl_answering(monkeypatch, *, returncode: int, stdout: str = "", stderr: str = "") -> list[list[str]]:
    ...
    def fake_run(arguments: list[str], **kwargs) -> subprocess.CompletedProcess:
        ...
        return subprocess.CompletedProcess(args=arguments, returncode=returncode, stdout=stdout, stderr=stderr)
...
class TestCreateIfAbsent:
    ...
    def test_refuses_to_read_any_other_failure_as_idempotence(self, monkeypatch):
        ...
        with pytest.raises(AssertionError, match="Could not create"):
