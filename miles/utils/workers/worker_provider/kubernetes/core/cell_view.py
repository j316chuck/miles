...
def _compute_worker_info(worker: KubernetesWorkerInfo, *, run: KubernetesRunInfo) -> WorkerInfo:
    ...
    assert isinstance(spec, ServeWorkerSpec), f"pool {pool_id} has no worker class, so its rpc methods are unknown"
    ...
    return WorkerInfo(
        ...
        handle=build_rpc_handle(worker_class=load_function(spec.worker_class), addrs=addrs, pool_id=pool_id),
    )
