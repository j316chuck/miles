...
class RayTrainGroup:
    ...
    async def create(
        ...
        *,
        ...
        provider: BaseWorkerProvider = RayWorkerProvider.create(spec_names=[group._spec_name])
        group._watcher_disposer = await provider.watch_cells(group._reconcile)
    ...
    def get_cell_statuses(self) -> dict[str, CellStatus]:
        return {
            compute_cell_id(spec_name=self._spec_name, cell_index=cell.cell_index): cell.cell_status()
            for cell in self._cells
        }
    ...
    async def _gather_all_alive_and_catch(self, compute_coroutine, *, debug_name: str, check_recoverable: bool = True):
        ...
        if check_recoverable and not self._is_recoverable():
            cause = _first_exception(outputs)
