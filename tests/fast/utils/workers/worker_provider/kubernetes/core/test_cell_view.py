...
def make_parsed_pod(*, pod_in_cell_index: int, cell_size: int = 2, ready: bool = True, **kwargs) -> ParsedPod:
    return ParsedPod(
        ...
        ready=ready,
        pod_ip=f"10.0.0.{pod_in_cell_index + 1}",
...
def make_run(*, workers_per_pod: int = 1) -> KubernetesRunInfo:
    return KubernetesRunInfo(
        namespace="rl",
        label_selector="app.kubernetes.io/instance=r",
        specs={
            "engine": make_pool_spec(
                "engine", ports={"rpc": 8000}, worker_class=ENGINE_CLASS, workers_per_pod=workers_per_pod
            )
        },
