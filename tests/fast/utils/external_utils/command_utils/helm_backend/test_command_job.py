...
class FakeKubectl:
    ...
    def __call__(self, arguments: list[str], *, input: str | None = None) -> subprocess.CompletedProcess:
        self.calls.append(arguments)
        if arguments[:2] == ["get", "pods"]:
            items = [
                {"metadata": {"name": f"convert-{index}", "labels": {command_job._COMPLETION_INDEX_KEY: str(index)}}}
...
def _run(
    ...
    monkeypatch.setattr(command_job, "_apply", lambda manifest, namespace: None)
    return command_job._run_job(
        _job(completions=completions),
...
def _render(monkeypatch: pytest.MonkeyPatch, **overrides: Any) -> list[str]:
    ...
    monkeypatch.setattr(command_job.helm, "run", fake_helm_run)
...
class TestRenderJob:
    ...
    def test_gives_the_job_the_same_deadline_the_launcher_waits_for(self, monkeypatch):
        ...
        assert "command_job.activeDeadlineSeconds=60" in _render(monkeypatch, timeout_seconds=60)
    ...
    def test_sends_the_command_as_json_so_helm_never_splits_it(self, monkeypatch):
        ...
        assert arguments[arguments.index("--set-json") + 1] == 'command_job.command=["bash", "-c", "echo a,b"]'
...
class TestRunOnNodes:
    ...
    def test_points_every_pod_at_the_headless_address_of_rank_zero(self, monkeypatch):
        ...
        assert calls[0]["command"][-1] == f"--master-addr {_job(step='gpu').master_address}"
    ...
    def test_carries_the_context_settings_into_the_job(self, monkeypatch):
        ...
        assert calls[0]["helm_values_files"] == ["/infra.yaml"]
...
class TestExecCommandGpu:
    ...
    def test_returns_the_single_result_rather_than_a_list(self, monkeypatch):
        ...
        monkeypatch.setattr(command_job, "_run_job", lambda **kwargs: ["the output"])
...
class TestRunJob:
    ...
    def test_selects_the_pods_of_this_job_alone(self, monkeypatch):
        ...
        assert listing[listing.index("--selector") + 1] == f"{command_job._JOB_NAME_LABEL}=miles-run-command-convert"
    ...
    def test_hands_its_own_timeout_to_the_rendered_job(self, monkeypatch):
        ...
        command_job._run_job(
            _job(timeout_seconds=90.0),
            command=["bash", "-c", "convert"],
            capture_output=False,
        ...
        assert int(rendered[0].timeout_seconds) == 90
